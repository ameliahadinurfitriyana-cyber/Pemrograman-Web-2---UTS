...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.customers.store') }}" class="max-w-lg">
        @csrf
        ...
        <div class="form-control mb-4">
            ...
            <textarea id="address" name="address" class="textarea textarea-bordered w-full">{{ old('address') }}</textarea>
        </div>

        <div class="form-control mb-4 flex flex-row gap-2">
            <button type="submit" class="btn btn-primary flex-1">Create Customer</button>
